create or replace function app_api.normalize_job_work_scope(p_value text)
returns text
language plpgsql
immutable
as $$
...
begin
  ...
  return regexp_replace(v_trimmed, '[[:space:]]+', ' ', 'g');
end;
$$;
...
immutable
as $$
  select app_api.normalize_job_work_scope(p_value);
$$;
