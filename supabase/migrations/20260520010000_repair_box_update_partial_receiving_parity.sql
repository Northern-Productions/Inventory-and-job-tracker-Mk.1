/**
 * PURPOSE:
 * Repairs live schema drift where app_api.build_box_from_payload was recorded
 * past 0044/20260407153000 but still used the older first received-film
 * validation path. Existing received boxes must be able to save partial
 * receiving metrics without forcing global FILM DATA seeding.
 *
 * AFFECTS:
 * /boxes/update for existing received boxes, including Phase 3C fixture box
 * checkout baseline setup. This migration reasserts the checked-in latest
 * build_box_from_payload semantics only; it does not update app data.
 */
