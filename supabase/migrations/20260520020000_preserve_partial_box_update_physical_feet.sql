/**
 * PURPOSE:
 * Preserves raw physical feet when an existing received box saves partial
 * receiving metrics without an explicit CurrentFeetOnRoll value.
 *
 * AFFECTS:
 * /boxes/update for existing received boxes, including Phase 3C fixture box
 * checkout baseline setup. This migration reasserts build_box_from_payload
 * semantics only; it does not update app data.
 */
...
BEGIN
  ...
  IF v_def NOT LIKE '%v_use_partial_receiving_metrics boolean := false%' THEN
    RAISE EXCEPTION 'build_box_from_payload missing partial receiving metrics declaration';
  END IF;

  IF v_def NOT LIKE '%if not v_has_full_receiving_metrics and p_existing_box_id is not null then%' THEN
    RAISE EXCEPTION 'build_box_from_payload missing existing-box partial receiving metrics branch';
  END IF;

  IF v_def LIKE '%if v_initial_weight_input is null and v_existing.initial_weight_lbs is null then%' THEN
    RAISE EXCEPTION 'build_box_from_payload still has stale first-save InitialWeightLbs guard';
  END IF;

  IF v_def NOT LIKE '%app_api.physical_film_commitment_feet_for_box(%' THEN
    RAISE EXCEPTION 'build_box_from_payload missing physical film commitment calculation';
  END IF;

  IF v_def NOT LIKE '%coalesce(v_existing.feet_available, v_feet_available)%' THEN
    RAISE EXCEPTION 'build_box_from_payload does not preserve existing physical feet for partial receiving updates';
  END IF;

  IF v_def LIKE '%v_feet_available := app_api.clamp_feet_to_initial_range(v_feet_available, v_initial_feet);%' THEN
    RAISE EXCEPTION 'build_box_from_payload still overwrites partial receiving physical feet from payload feetAvailable';
  END IF;
END;
$$;
