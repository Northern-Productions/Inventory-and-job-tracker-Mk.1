...
export function WarehouseToggle({ value, onChange }: WarehouseToggleProps) {
  return (
    <div className="toggle-group warehouse-toggle-group" role="tablist" aria-label="Warehouse">
      {(['IL', 'MS'] as const).map((warehouse) => (
        <button
          key={warehouse}
          type="button"
          className={`toggle-button warehouse-toggle-button ${
            value === warehouse ? 'toggle-button-active' : ''
          }`.trim()}
