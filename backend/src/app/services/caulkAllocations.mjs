...
export async function removeCaulkAllocation(client, orgId, actor, payload) {
  await requireAllocationWriteAccess(client, orgId);
  ...
  if (openCheckoutCount > 0) {
    throw new HttpError(
      400,
      `Caulk allocation ${caulkAllocationId} has ${openCheckoutCount} open checkout${openCheckoutCount === 1 ? '' : 's'}. Check in first.`
    );
  }
  ...
  const reservedTubesRemaining = integerOrZero(allocation.reserved_tubes_remaining);
  const removalReason =
    asTrimmedString(payload?.reason) || `Removed from job ${asTrimmedString(allocation.job_number)}.`;
  const warnings = [];
  const pendingTransfer = await findLockedPendingTransferByAllocationRowId(client, orgId, allocation.id);

  if (pendingTransfer) {
    const transferCancellation = await cancelPendingCaulkTransferInternal(
      client,
      orgId,
      normalizedActor,
      pendingTransfer,
      `Cancelled pending transfer ${asTrimmedString(pendingTransfer.transfer_id)} while removing allocation ${caulkAllocationId}.`
    );
    warnings.push(...transferCancellation.warnings);
  }
  ...
  await client.query(
    `
      ...
      where id = $1::uuid
        and org_id = $2::uuid
    `,
    [allocation.id, orgId, normalizedActor, removalReason]
  );
  ...
  return {
    result: {
      jobNumber: asTrimmedString(allocation.job_number),
      caulkAllocationId,
      releasedReservedTubes: reservedTubesRemaining,
      warnings,
    },
    warnings,
  };
