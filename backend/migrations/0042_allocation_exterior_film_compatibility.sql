...
as $$
  with canonical as (
    select app_api.trim_text(app_api.resolve_canonical_film_name(p_org_id, p_manufacturer, p_film_name)) as film_name
  )
  select app_api.normalize_catalog_manufacturer_lookup_key(p_manufacturer)
    || '|' || app_api.normalize_catalog_lookup_key(
      case
        when canonical.film_name ~* '(^|[[:space:]])Exterior[[:space:]]*$' then
          coalesce(
            nullif(regexp_replace(canonical.film_name, '[[:space:]]+Exterior[[:space:]]*$', '', 'i'), ''),
            canonical.film_name
          )
        else canonical.film_name
      end
    )
  from canonical;
$$;
...
stable
as $$
  select app_api.trim_text(app_api.resolve_canonical_film_name(p_org_id, p_manufacturer, p_film_name))
...
stable
as $$
  select app_api.normalize_requirement_film_family_key(
      p_org_id,
    ...
    and (
      not app_api.requirement_film_is_exterior(
        p_org_id,
        p_requirement_manufacturer,
        p_requirement_film_name
      )
      or app_api.requirement_film_is_exterior(
        p_org_id,
        p_candidate_manufacturer,
        p_candidate_film_name
      )
