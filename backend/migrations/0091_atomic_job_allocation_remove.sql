/**
 * PURPOSE:
 * Moves job film allocation removal into one database transaction so an
 * allocation cannot be cancelled while a later box or film-order update fails.
 *
 * AFFECTS:
 * POST /allocations/remove-box, job allocation detail, film requirement
 * remaining LF, box planning capacity, linked film order state, and
 * AUTO_PLANNED suppression/reconciliation.
 *
 * WHEN CHANGING THIS, ALSO CHECK:
 * Supabase Edge mutationHandlers /allocations/remove-box, backend schema latest
 * checks, verify-ordered-box-allocation-flow, and frontend allocation row tests.
 *
 * COMMON FAILURE MODES:
 * Removing by box id instead of allocation id, direct PostgREST updates without
 * scoped filters, partial success after allocation cancellation, or re-planning
 * a user-removed AUTO_PLANNED row.
 */
...
create or replace function public.api_acl_allocations_remove_box(
  p_org_id uuid,
  p_actor text,
  p_payload jsonb
)
...
comment on function public.api_allocations_remove_box(uuid, text, jsonb)
  is 'Atomically removes one job film allocation by allocation id and recalculates related capacity, film order, and planner state.';

comment on function public.api_acl_allocations_remove_box(uuid, text, jsonb)
  is 'Access-controlled wrapper for atomic job film allocation removal.';

select app_api.grant_execute_if_exists('public.api_acl_allocations_remove_box(uuid, text, jsonb)', 'authenticated');
select app_api.grant_execute_if_exists('public.api_acl_allocations_remove_box(uuid, text, jsonb)', 'service_role');
