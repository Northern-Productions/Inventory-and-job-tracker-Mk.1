create or replace function app_api.assert_can_checkout_box_from_warehouse(p_box app.boxes)
returns void
language plpgsql
stable
security definer
...
as $$
begin
  if upper(coalesce(p_box.status::text, '')) = 'IN_STOCK'
     and not app_api.box_has_weight_baseline(p_box) then
    perform app_api.raise_http(
      400,
      format(
        'Box %s must be weighed and have a saved Last Roll Weight before it can be checked out from warehouse inventory.',
        upper(coalesce(p_box.box_id, ''))
      )
    );
  end if;
end;
