...
test('audit checkout projection source avoids note parsing, migrations, and duplicate behavior changes', async () => {
  ...
  assert.match(schemaLatest, /0152_fix_planner_suppression_on_conflict\.sql/);
  ...
  assert.equal(await latestBackendMigration, '0152_fix_planner_suppression_on_conflict.sql');
  assert.equal(
