...
test('current repository chain through 0200 is coherent with explicit legacy warnings', () => {
  ...
  assert.equal(latest.logicalId, '0200');
  assert.equal(latest.supabaseVersion, '20260814100000');
...
test('CLI check and JSON modes are deterministic and privacy-safe', () => {
  ...
  assert.equal(JSON.parse(first.stdout).latest.logicalId, '0200');
