...
test('current repository chain through 0202 is coherent with explicit legacy warnings', () => {
  ...
  assert.equal(latest.logicalId, '0202');
  assert.equal(latest.supabaseVersion, '20260814210000');
...
test('CLI check and JSON modes are deterministic and privacy-safe', () => {
  ...
  assert.equal(JSON.parse(first.stdout).latest.logicalId, '0202');
