...
test('current repository chain through 0205 is coherent with explicit legacy warnings', () => {
  ...
  assert.equal(latest.logicalId, '0205');
  assert.equal(latest.supabaseVersion, '20260824100000');
...
test('CLI check and JSON modes are deterministic and privacy-safe', () => {
  ...
  assert.equal(JSON.parse(first.stdout).latest.logicalId, '0205');
